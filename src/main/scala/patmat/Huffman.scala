   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
		if (chars.isEmpty) Nil
			...
			List((chars.head,chars.count(x=>x==chars.head))) ::: times(chars.filterNot(x=>x==chars.head))
		}
	}
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
		if (freqs.isEmpty) Nil
			...
			List(new Leaf(freqs.head._1,freqs.head._2)) ::: makeOrderedLeafList(freqs.tail) sortWith (_.weight < _.weight)
		}
	}
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
		def lookup(t: CodeTree)(ch: Char): List[Bit] = t match {
			case Leaf(_, _) => List()
			case Fork(l, r, _, _) => if (chars(l).contains(ch)) List(0) ::: lookup(l)(ch) else List(1) ::: lookup(r)(ch)
		}
